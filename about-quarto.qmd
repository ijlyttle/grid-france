...
[Quarto's default rendering strategy](https://quarto.org/docs/projects/quarto-projects.html#render-targets) is to render all the files it can find in a directory, generally in alphabetical order. For a data pipeline, we want to render only those files that need updating, and to render in the optimal order. This is where DVC comes in, to determine the execution order.
...
**TODO** - write about running `quarto render` after `dvc repro` in order to sweep up the published files into the artifact.
...
This is why this project uses `.qmd` files, rather than more-familiar `.ipynb` files.

There is hope, however, for using `.ipynb` files in DVC. There is an [issue at the DVC repo](https://github.com/iterative/dvc/issues/3439#issuecomment-663873682) asking about the possibility to filter a source file to determine the dependency. In our case we would want to filter to keep all the code cells in a notebook, possibly using `nbconvert`.